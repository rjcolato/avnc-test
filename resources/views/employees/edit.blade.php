...
<div class="row">
    <div class="col-2"></div>
    <div class="col-8">
        <form class="row g-3" action="{{ route('employees.update',  $elemento->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="mb-3">
                <div class="form-group">
                    <strong>Nombres:</strong>
                    <input type="text" name="first_name" class="form-control" placeholder="Primer Nombre"
                    value="{{$elemento->first_name}}">
                    @error('first_name')
                    ...
                    <select class="form-select" aria-label="Default select example" name="branches_id">
                        ...
                        @foreach($branches as $branches)
                            <option value="{{$branches->id}}"
                                {{isSelected($branches->id, $elemento->branches)}}>
                                {{$branches->name}}
                        @endforeach
                    </select>
                    @error('branches_id')
           ...
           <div class="mb-3">
            ...
            <div class="mb-3">
                <div class="form-group">
                    ...
                    <option value="{{$gengre->id}}"
                        {{isSelected($gengre->id, $elemento->gengre_id)}}>
                        {{$gengre->name}}
                    </option>
                @endforeach
                        <option selected>--seleccione uno--</option>
